...
const updatePrograms = async function(uid, json){
  ...
  try{
    ...
    const collection = firestore.collection("/users").doc(uid.toString()).collection('programs')
    ...
    programs.forEach(async (program) => {
      ...
      const funcToCall = async (val) => {
        let doc = {}
        try{
          doc = await firestore.collection("/users").doc(uid.toString()).collection('programs').doc(val).get()

        } catch(err){
          console.log(`"${val}" program for ${uid} not found, failed to get doc...`)
          console.log(err)
        }
        if(doc.exists){
          ...
          console.log(`"${val}" program for ${uid} not found, ignoring update...`)
        }
      }
