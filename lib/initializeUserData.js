...
const initializeUserData = async function(uid, options) {
  ...
  try {
    ...
    //       .set(createDefaultProgramDoc(lang, index)),
    //   );
    // });
    Object.keys(dayOfCodePrograms).forEach(function(key) {
      promises.push(programCollection.doc(key).set(dayOfCodePrograms[key]));
    });
