...
const createUser = async function(userInfo){
    ...
    if(!userInfo){
        ...
        return {ok:false, data:{code:"missing", message:"No user info provided..."}}
    }
    ...
    if(!uid || !password || !displayName){
        ...
        return {ok:false, data:{code:"missing", message:"Missing required information in userInfo..."}}
    }
    ...
    try{
        ...
        console.log(err)
        return {ok:false, data:{code:"catch-fail", message:err.message ? err.message : "Failed to create user"}}
    }

    return {ok:false, data:{code:"catch-fail", message:"Got past try-catch"}}
}
