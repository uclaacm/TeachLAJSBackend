...
const getProgram = async function(sketchid) {
  ...
  try {
    ...
    try {
      ...
      if (progRes.exists) {
        winston.verbose(`Found program with id ${sketchid}`);
        program = progRes.get();
        result = { ok: true, data: program };
      } else {
        winston.error(`No program document with id ${sketchid} found.`);
        return { ok: false, error: "No program document of the given id was found!" };
      }
    } catch (err) {
