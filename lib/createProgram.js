...
const NUM_PROGRAM_THUMBNAIL_OPTIONS = require("../constants").NUM_PROGRAM_THUMBNAIL_OPTIONS;
const languageToDefaultCode = require("../constants/helpers.js").languageToDefaultCode;
...
const createProgram = async function(body) {
  ...
  if (!code) {
    code = languageToDefaultCode(language);
  }
  ...
  try {
    ...
    return { ok: true, data: { programData: programData, key: newProgramDoc.id } };
  } catch (err) {
